...
function [finalLabel e]= BCAlgo( im, fixedBG,  K, G, maxIterations, Beta, diffThreshold )
...
for iter=1:maxIterations
    ...
    [gch e] = GraphCut('energy', graphHandle);
%     %%%% get current total energy
%     fprintf('before cut, total energy: %f \n', e); 
    
    [graphHandle currLabel] = GraphCut('expand', graphHandle);
    
%     [gch e] = GraphCut('energy', graphHandle);
%     %%%% get current total energy
%     fprintf('after cut, total energy: %f \n', e); 
    
    currLabel = 1 - currLabel;
    GraphCut('close', graphHandle);
    %%%% if all object or background, break the loop
    if (length(find(currLabel==1))<100)||(length(find(currLabel==0))<100)
        break;
    end
...
%     % [fgClusterIds,fgMean] = kmeans(fgValues, K+1, 'start', fgMeanInit,  'emptyaction','singleton', 'Options',opts);
%     [bgClusterIds,bgMean] = kmeans(bgValues, K,  'emptyaction','singleton' ,'Options',opts);
%     [fgClusterIds,fgMean] = kmeans(fgValues, K+1,  'emptyaction','singleton', 'Options',opts);
% else
    [bgClusterIds,bgMean] = kmeans(bgValues, K, 'emptyaction','singleton' ,'Options',opts);
    [fgClusterIds,fgMean] = kmeans(fgValues, K+1, 'emptyaction','singleton', 'Options',opts);
% end

%%%%%%%%%%%% Minimize the similarity of Obj GMM and Bkg GMM
NearestNeighbor=zeros(1+K,1);
for k=1:1+K
    FOneCluster=fgMean(k,:);
    Distance=sum(abs(repmat(FOneCluster,K,1)-bgMean),2);
    Similarity=1/min(Distance);
    NearestNeighbor(k)=Similarity;
end
[Sorted,fgClusterIds_wanted] = sort(NearestNeighbor); % 'ascend' (default) 
DeleteClusterWeight = nnz(fgClusterIds==fgClusterIds_wanted(K+1))/numFGValues;
if Sorted(end) > 1/50 && DeleteClusterWeight > 0.1  % 60 is the distance threshold in RGB space, delete when similarity is high and weight is big
    fgClusterIds_wanted=fgClusterIds_wanted(1:K); % cluster 1-6, maybe 3 is too similar to bkg, so fgClusterIds_wanted becomes 1,2,4,5,6  
else
   if ( ~isempty(bgMeanInit) && ~isempty(fgMeanInit) )
...
for k=1:K
    ...
    fgGaussianWeight = nnz(fgClusterIds==fgClusterIds_wanted(k))/numFGValues;
    ...
    fgDist = imageValues - repmat(fgMean(fgClusterIds_wanted(k),:),size(imageValues,1),1);
    ...
    fgCovarianceMat = cov(fgValues(fgClusterIds==fgClusterIds_wanted(k),:));
