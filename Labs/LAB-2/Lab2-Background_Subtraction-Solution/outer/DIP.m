  ...
  methods(Static=true)
    ...
    function ComputeBkg(in_path, in_format, outpath)
        ...
        for i=1:ImageNum
            ...
            bkg_R = bkg_R + double(I(:,:,1));
            bkg_G = bkg_G + double(I(:,:,2));
            bkg_B = bkg_B + double(I(:,:,3));
        end
        bkg_R = uint8(bkg_R./ImageNum);
        bkg_G = uint8(bkg_G./ImageNum);
        bkg_B = uint8(bkg_B./ImageNum);
        bkg = cat(3, bkg_R, bkg_G, bkg_B);
        imwrite(bkg,outpath);
    end
    %----------------------------------------------------------------------------
    ...
    function BkgSub(in_path, in_format, bkg_path, outpath)
        ...
        for i=1:ImageNum
            ...
            bkg = imread(bkg_path);
            bkg = rgb2gray(bkg);
            fg = uint8(abs(double(I) - double(bkg))); 
            fg = im2bw(fg,graythresh(fg)); % otsu adaptive thesholding 
            imwrite(fg,[outpath name '.' in_format]);
        end
    end
    %----------------------------------------------------------------------------
  end
